...
class Experiment():
	def __init__(self, numTrials=None, ISI=None, flash_dur=None, luminance=None, wave_freq=None, pulse_dur=None,
				 wave_amp=None, stimulus=None):
	...
	def run_experiment(self, window_dim=[2560, 1600], screen_index=0, monitor_name="testMonitor"):
		...
		flash = fs2.Flash_Stim(cur_screen=screen_index, cur_monitor=monitor_name, win_dim=window_dim, pres_dur=0,
							   luminance=0)
		flash.autoDraw = True
		...
		randNs = random.sample(range(0, len(stimList) - 1),
							   self.numTrials)  # generate random numbers equal to the # of trials

		trials_subset = [
							None] * self.numTrials  # since there are too many possiblilties we may only want a subset of them, so generate random numbers to choose which trials
		...
		trials = data.TrialHandler(trialList=trials_subset, nReps=1,
								   method='sequential')  # since already randomized we can go ahead and use "sequential" here...
...
def main():
	...
	exper = Experiment(numTrials=numTrials, ISI=ISI, flash_dur=flash_dur, luminance=luminance, wave_freq=frequency,
